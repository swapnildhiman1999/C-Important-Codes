...
#define ll long long
using namespace std;
void connectedComponents(vector<ll> *edges,ll n,bool *visited,ll sv,vector<ll> *component){
  visited[sv]=true;
  component->push_back(sv);
  for(ll i=0;i<edges[sv].size();i++){
    if(!visited[edges[sv][i]]){
      connectedComponents(edges,n,visited,edges[sv][i],component);
    }
  }
}
int main(){
  ...
  while(t--){
    ...
    vector<vector<ll>*>* components=new vector<vector<ll>*>();
    bool *visited=new bool[n];
    memset(visited,false,sizeof(visited));
    for(ll i=0;i<n;i++){
      if(!visited[i]){
        vector<ll> *component=new vector<ll>();
        connectedComponents(edges,n,visited,i,component);
        components->push_back(component);
      }
    }
    vector<vector<ll>*>::iterator it=components->begin();
    ll val=INT_MIN;
    while(it!=components->end()){
      vector<ll> *component=*it;
      vector<ll>::iterator it1=component->begin();
      ll value=0;
