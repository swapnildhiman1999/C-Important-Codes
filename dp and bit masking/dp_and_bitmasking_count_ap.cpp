...
#define modulus  100001
int numofAP(int *arr,int n){
 unordered_map<int,int>* map=new unordered_map<int,int>[n];
 //default value 0 is stored
  int  ans=1+n;
  // for AP of length 2
  for(int i=0;i<n-1;i++){
    for(int j=i+1;j<n;j++){
      int diff=arr[j]-arr[i];
      map[i][diff]=(map[i][diff]+1);
      ans=(ans+1)%modulus;
    }
  }
  //for AP of length >=3 starting at i th index
  for(int i=n-3;i>=0;i--){
    for(int j=i+1;j<n;j++){
      int diff=arr[j]-arr[i];
      map[i][diff]=(map[i][diff]+map[j][diff])%modulus;
      ans=(ans+map[j][diff])%modulus;
    }
  }
  delete[] map;
  return ans;
